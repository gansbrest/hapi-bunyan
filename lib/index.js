...
function register(server, options, next) {
  ...
  server.on('log', function(data, tags) {
    ...
    if (options.excludeEvents.indexOf('log') !== -1) return;
  ...
  server.on('request', function(request, data, tags) {
    ...
    if (options.excludeEvents.indexOf('request') !== -1) return;
  ...
  server.on('request-internal', function(request, data, tags) {
    ...
    if (options.excludeEvents.indexOf('request-internal') !== -1) return;
  ...
  server.on('request-error', function(request, err) {
    ...
    if (options.excludeEvents.indexOf('request-error') !== -1) return;
